import pygame
from sources.constants import WHITE, RED, BLACK, block_size, left_margin, right_margin, upper_margin, down_margin
from sources.constants import size, LETTERS, font_size, screen, font
from sources.field import Field
from sources.messeges import Messages
...
def main():
    ...
    ship_drawing.draw_ship_with_rules_first(human_ships_to_draw_first, num_ships_list, human_ships_to_draw,
                                            human_ships_set, count, final_mes, ship_not_created, drawing,
                                            length_of_remaining_ships, field_ship_first)
    screen.fill(BLACK)
    num_ships_list = [0] * 4
    screen.fill(BLACK)
    human_ships_to_draw = []
    human_ships_set = set()
    count = 11
    ship_drawing.draw_ship_with_rules_second(human_ships_to_draw_second,num_ships_list, human_ships_to_draw,
                                             human_ships_set, count,final_mes, ship_not_created, drawing,
                                             length_of_remaining_ships, field_ship_second)
                ...
                if (left_margin < x < left_margin + 10 * block_size) and (upper_margin < y < upper_margin + 10 * block_size):
                    ...
                    if fired_block in human_ships_to_draw_second:
                        ...
                        if (length_of_remaining_ships[field_ship_second[fired_block[0]][fired_block[1]]] == 0):
                            ship_drawing_k.draw_ship_is_kill_end(fired_block, 0, dotted_set_first, field_ship_second)
                        hit_blocks_first.add(fired_block)
                        ship_drawing_k.draw_hit_blocks(hit_blocks_first)
                        if len(opponents_ships_list_first) == 0:
                            final_mes.shoot_text_delete(0)
                            final_mes.shoot_text_delete(15 * block_size)
                            final_mes.game_over_name("The first player won")
                            end_game = True
                            pygame.display.update()
                    else:
                        dotted_set_first.add(fired_block)
                        ship_drawing_k.draw_from_dotted_set(dotted_set_first)
                        computer_turn = True
                pygame.display.update()
            elif computer_turn and event.type == pygame.MOUSEBUTTONDOWN:

                x, y = event.pos
                if (left_margin + 15 * block_size < x < left_margin + 25 * block_size) and (upper_margin < y < upper_margin + 10 * block_size):
                    fired_block = ((x - left_margin) // block_size + 1,
                                   (y - upper_margin) // block_size + 1)
                    if fired_block in human_ships_to_draw_first:
                        ...
                        if (length_of_remaining_ships[field_ship_first[fired_block[0]][fired_block[1]]] == 0):
                            ship_drawing_k.draw_ship_is_kill_end(fired_block, 15, dotted_set_second, field_ship_first)
                        hit_blocks_second.add(fired_block)
                        ship_drawing_k.draw_hit_blocks(hit_blocks_second)
                        if len(opponents_ships_list_second) == 0:
                            final_mes.shoot_text_delete(0)
                            final_mes.shoot_text_delete(15 * block_size)
                            final_mes.game_over_name("The second player won")
                            end_game = True
                            pygame.display.update()

                    else:
                        dotted_set_second.add(fired_block)
                        ship_drawing_k.draw_from_dotted_set(dotted_set_second)
                        computer_turn = False
                pygame.display.update()
main()
pygame.quit()